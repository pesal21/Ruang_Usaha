...
<body class="bg-white text-gray-800">
    ...
    <main class="max-w-4xl mx-auto px-6 pt-24 pb-20">
        ...
        <!-- BACK -->
        <div class="mb-6">
            <a href="{{ route('umkm.pilih') }}" class="inline-flex items-center gap-1 text-blue-600 text-sm hover:text-blue-700 hover:underline transition-colors duration-200">
                < Kembali
                    </a>
        </div>

        <!-- HEADER CARD -->
        <div class="border border-gray-200 rounded-2xl p-6 mb-10 hover:shadow-md transition-shadow duration-300">
            <div class="flex gap-5">
                ...
                <div class="flex-shrink-0">
                    ...
                    <img src="{{ asset('storage/'.$umkm->logo) }}"
                        class="w-28 h-28 rounded-xl object-cover border border-gray-100 shadow-sm">
                    @else
                    <div class="w-28 h-28 bg-gray-100 rounded-xl flex items-center justify-center text-gray-400 text-sm">
                        Logo
                    </div>
                    @endif
                </div>
                ...
                <div class="flex-1 min-w-0">
                    <h1 class="text-2xl font-bold text-gray-900 mb-2">{{ $umkm->nama_usaha }}</h1>
                    <p class="text-gray-600 text-sm leading-relaxed mb-4 text-justify">{{ $umkm->deskripsi }}</p>
            ...
            {{-- Edit/Kelola buttons (only for owner) --}}
            @auth
            @if(auth()->user()->id === $umkm->user_id)
            <div class="flex gap-3 mt-5 pt-5 border-t border-gray-100">
                <a href="{{ route('umkm.edit', $umkm->id) }}"
                    class="px-4 py-2 border border-gray-200 rounded-lg text-sm hover:bg-gray-50 hover:border-gray-300 active:scale-95 transition-all duration-200">
                    Edit Profil
                </a>
                <a href="{{ route('umkm.kelola', $umkm->id) }}"
                    class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 active:scale-95 transition-all duration-200 shadow-sm hover:shadow-md">
                    Kelola Produk
                </a>
            </div>
            @endif
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-12">
            @forelse($produk as $p)
            <div class="bg-white border border-gray-100 rounded-xl overflow-hidden shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
                @if($p->foto_produk)
                <img src="{{ asset('storage/'.$p->foto_produk) }}"
                    class="w-full h-32 object-cover">
                @else
                <div class="w-full h-32 bg-gray-100 flex items-center justify-center text-gray-400 text-xs">
                    Tidak ada foto
                </div>
                @endif
                <div class="p-2.5">
                    <p class="text-xs font-semibold text-gray-800 truncate">{{ $p->nama_produk }}</p>
                    <p class="text-xs text-gray-400">{{ $p->kategori ?? 'Makanan' }}</p>
            ...
            @empty
            <p class="text-gray-400 text-sm col-span-full">Belum ada produk</p>
            @endforelse
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-12">
            ...
            <div class="overflow-hidden rounded-2xl">
                <img src="{{ asset('storage/'.$foto->foto) }}"
                    class="w-full h-52 object-cover rounded-2xl hover:scale-105 transition-transform duration-300">
            </div>
            @endforeach
        </div>
        @endif
