...
<head>
    ...
    <title>Edit Produk – RuangUsaha</title>
...
<body class="bg-gray-50 text-gray-800">
    ...
    <main class="pt-24 pb-20 px-4">
        ...
        <div class="max-w-6xl mx-auto mb-4">
            <a href="{{ route('umkm.kelola', $produk->umkm_id) }}" class="inline-flex items-center gap-1 text-blue-600 text-sm hover:underline">
                < Kembali
                    </a>
        </div>
        ...
        <div class="max-w-6xl mx-auto bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            ...
            <form action="{{ route('produk.update', $produk->id) }}" method="POST" enctype="multipart/form-data" class="grid md:grid-cols-2 gap-8" id="produk-form">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" rows="4"
                            ...
                            class="w-full border border-gray-200 rounded-lg py-2.5 px-4 text-sm bg-gray-50
                                     focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition resize-none">{{ $produk->deskripsi }}</textarea>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="drag-area rounded-xl p-8 text-center cursor-pointer border-2 border-dashed border-gray-300"
                        ...
                        <div id="uploadPlaceholder">
                            ...
                            <p class="text-xs text-gray-400 mt-2">PNG, JPG, GIF (MAX 800x400px)</p>
                        </div>
                    ...
                    <p class="text-xs text-gray-500 mt-3">
                        Foto saat ini: <span class="text-gray-700 font-medium">{{ basename($produk->foto_produk) }}</span>
                    </p>
                    @endif
                </div>
            </form>
            ...
            <div class="flex justify-end gap-3 mt-8 pt-6 border-t">
                <a href="{{ route('umkm.kelola', $produk->umkm_id) }}"
    ...
    <script>
        ...
        document.addEventListener('click', function(event) {
            const dropdown = document.getElementById('profileDropdown');
            if (!event.target.closest('[onclick="toggleDropdown()"]') && !dropdown.contains(event.target)) {
        ...
        // Show existing image if available on page load
        window.addEventListener('DOMContentLoaded', () => {
            const existingImage = "{{ $produk->foto_produk ? asset('storage/'.$produk->foto_produk) : '' }}";
            if (existingImage) {
                document.getElementById('uploadPlaceholder').classList.add('hidden');
                document.getElementById('uploadPreview').classList.remove('hidden');
                document.getElementById('previewImg').src = existingImage;
                document.getElementById('fileName').textContent = "{{ basename($produk->foto_produk) }}";
            }
        });
    </script>
